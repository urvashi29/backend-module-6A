...
-- truncate (delete all the data from table)
truncate table movies;

-- delete the table
drop table movies;

-- creating user table with constraint 
...
-- try the age constraint
-- insert into users values(1, "Alina", 15, "alina@gmail.com", "10-10-25");

-- try running to check primary key & unique constraint
insert into users values(1, "Alina", 20, "alina@gmail.com", "10-10-25");

-- default curdate is added
insert into users(Id, name, age, email) values(2, "Alex", 25, "alex@gmail.com");

-- try running to check unique constraint
insert into users(Id, name, age, email) values(3, "Alex", 25, "alex@gmail.com");

-- try to putting null value
-- insert into users(Id, name, age, email) values(null, "Alaya", 30, null);
insert into users(Id, name, age, email) values(4, "Alaya", 30, "alaya@gmail.com");
...
insert into users(name, age, email) values("Alex", 20, "admin@gmail.com")

insert into users(name, age, email) values("Alaya", 24, "alaya@gmail.com")
...
-- select all the distinct column
select distinct last_name from customers;

-- order by ascending order alphabetically (asc)
select last_name, first_name from customers order by last_name;

-- order by descending order alphabetically 
select last_name, first_name from customers order by last_name desc;

-- order by 2 or more column
select last_name, first_name from customers order by first_name, last_name;

-- limit
select * from customers limit 5

-- order by first_name and get first 5 records
select * from customers order by first_name limit 10;
...
-- return number count of products row
select count(*) from products;

-- sum
select sum(price) from products;

-- returning thr sum value of all the price for record where stock_quantity = 50
select sum(price) from products where stock_quantity = 50;

-- avg
select avg(price) from products;

-- return the price of the product with maximum price
select max(price) from products;

-- return the price of the product with minumum price
select min(price) from products;

-- group by
select stock_quantity, sum(price) from products group by stock_quantity; 

-- retunr the count of customers with same grouped address
...
-- get the records in pattern e.g. name start should be "Ja"
select * from employee where first_name like "Ja%";

-- e.g. name contain  "Ja"
select * from employee where first_name like "%Ja%";

-- "_" means any 1 character e.g return all the records contains any character followed by  "an"
select * from employee where first_name like "_an%";

-- e.g return all the records contains "a" followed by any character followed by "n"
select * from employee where first_name like "%o_n%";
